...
typedef enum {
	...
	MTY_EVENT_SCALE          = 27, ///< The scale factor has been changed.
...
typedef struct MTY_SWCursor MTY_SWCursor;

/// @brief Create a cursor context.
/// @param ctx The MTY_SWCursor.
/// @param window The target window.
/// @param id The cursor identifier.
/// @returns The newly created cursor context.
MTY_EXPORT MTY_SWCursor *
MTY_SWCursorCreate(MTY_App *app, MTY_Window window, uint32_t id);

/// @brief Destroy the cursor context.
/// @param ctx The MTY_SWCursor.
MTY_EXPORT void
MTY_SWCursorDestroy(MTY_SWCursor **cursor);

/// @brief Enable or disable cursor drawing.
/// @details When disabled, MTY_SWCursorDraw() has not effect.
/// @param ctx The MTY_SWCursor.
/// @param enable True to draw the cursor, false otherwise.
MTY_EXPORT void
MTY_SWCursorEnable(MTY_SWCursor *ctx, bool enable);
...
/// @param hotY The cursor's vertical hotspot position.
MTY_EXPORT void
MTY_SWCursorSetHotspot(MTY_SWCursor *ctx, uint16_t hotX, uint16_t hotY);
...
MTY_EXPORT void
MTY_SWCursorSetImage(MTY_SWCursor *ctx, const void *data, size_t size);

/// @brief Set the window to which the cursor belongs.
/// @param ctx The MTY_SWCursor.
/// @param window The new window the cursor belongs to.
MTY_EXPORT void
MTY_SWCursorSetWindow(MTY_SWCursor *ctx, MTY_Window window);
...
MTY_EXPORT void
MTY_SWCursorMove(MTY_SWCursor *ctx, int32_t x, int32_t y, float scale);

/// @brief Move the cursor based on the current MTY_Zoom state.
/// @param ctx The MTY_SWCursor.
/// @param zoom The MTY_Zoom.
MTY_EXPORT void
MTY_SWCursorMoveFromZoom(MTY_SWCursor *ctx, MTY_Zoom *zoom);

/// @brief Check if the cursor has moved since the previous call.
/// @param ctx The MTY_SWCursor.
/// @returns True if the cursor has moved, false otherwise.
MTY_EXPORT bool
MTY_SWCursorHasMoved(MTY_SWCursor *ctx);
...
MTY_SWCursorDraw(MTY_SWCursor *ctx, MTY_DrawData *dd);
