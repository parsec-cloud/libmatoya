...
MTY_SWCursor *MTY_SWCursorCreate(MTY_App *app, MTY_Window window, uint32_t id)
{
	MTY_SWCursor *ctx = MTY_Alloc(1, sizeof(MTY_SWCursor));
...
void MTY_SWCursorDestroy(MTY_SWCursor **cursor)
{
	if (!cursor || !*cursor)
		return;

	MTY_SWCursor *ctx = *cursor;
...
void MTY_SWCursorEnable(MTY_SWCursor *ctx, bool enable)
{
	VALIDATE_CTX(ctx);
...
void MTY_SWCursorSetHotspot(MTY_SWCursor *ctx, uint16_t hotX, uint16_t hotY)
{
	VALIDATE_CTX(ctx);
...
void MTY_SWCursorSetImage(MTY_SWCursor *ctx, const void *data, size_t size)
{
	VALIDATE_CTX(ctx);
...
void MTY_SWCursorSetWindow(MTY_SWCursor *ctx, MTY_Window window)
{
	VALIDATE_CTX(ctx);
...
void MTY_SWCursorMove(MTY_SWCursor *ctx, int32_t x, int32_t y, float scale)
{
	VALIDATE_CTX(ctx);
...
void MTY_SWCursorMoveFromZoom(MTY_SWCursor *ctx, MTY_Zoom *zoom)
{
	VALIDATE_CTX(ctx);
	...
	MTY_SWCursorMove(ctx, cursor_x, cursor_y, scale);
}

bool MTY_SWCursorHasMoved(MTY_SWCursor *ctx)
{
...
MTY_DrawData *MTY_SWCursorDraw(MTY_SWCursor *ctx, MTY_DrawData *dd)
{
	VALIDATE_CTX(ctx, dd);
