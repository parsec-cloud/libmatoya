...
static void event_func(const MTY_Event *evt, void *opaque)
{
	...
	if (evt->type != MTY_EVENT_MOVE)
		MTY_PrintEvent(evt);

	if (evt->type == MTY_EVENT_CLOSE || (evt->type == MTY_EVENT_TEXT && evt->text[0] == 'q') || (evt->type == MTY_EVENT_KEY && evt->key.key == 0x1))
		ctx->quit = true;
}
...
int main(int argc, char **argv)
{
	...
	MTY_Window window = MTY_WindowCreate(ctx.app, "My Window", NULL, 0);
	MTY_WindowSetGFX(ctx.app, window, MTY_GFX_D3D11, true);
