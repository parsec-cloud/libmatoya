...
typedef enum {
	...
	MTY_EVENT_TOUCH          = 27, ///< Touch input event has occurred.
...
/// @brief Touch input contact state.
typedef enum {
	MTY_TOUCH_CONTACT_STATE_ONGOING = 0, ///< The touch input is currently in contact with the surface.
	MTY_TOUCH_CONTACT_STATE_BEGIN   = 1, ///< The touch input has just established contact with the surface.
	MTY_TOUCH_CONTACT_STATE_END     = 2, ///< The touch input has just ceased contact with the surface.
	MTY_TOUCH_CONTACT_STATE_MAKE_32 = INT32_MAX,
} MTY_TouchContactState;
...
typedef struct {
	uint8_t id;                  ///< Uniquely identifies the touch input in a touch sequence.
	bool primary;                ///< Determines whether this touch input is a primary input or not.
	                             ///< The "primary" touch input triggers mouse events. There can
	                             ///< only be one primary touch contact in a MTY_TouchEvent.
	uint16_t x;                  ///< The horizontal position in the client area of the window.
	uint16_t y;                  ///< The vertical position in the client area of the window.
	MTY_TouchContactState state; ///< Touch input contact state.
} MTY_TouchContact;

/// @brief Touch input event.
typedef struct {
	uint8_t count;                ///< The number of elements in the `contacts` array. Represents the
	                              ///< number of touch contacts in the event's touch sequence.
	MTY_TouchContact contacts[5]; ///< Touch contacts for the touch sequence represented by this event.
	                              ///< The order of contacts is guaranteed to be the order in which
	                              ///< contact was established. Note however that you cannot assume
	                              ///< any correlation between a contact's `id` and its index in this array.
} MTY_TouchEvent;
...
typedef struct MTY_Event {
	...
	union {
		...
		MTY_TouchEvent touch;           ///< Valud on MTY_EVENT_TOUCH.
