...
function mty_str_to_c(str, ptr, size) {
  ...
  if (buf.byteLength >= size) throw 'mty_str_to_c overflow';
...
function mty_buf_to_b64(buf) {
  let bstr = '';
  for (let x = 0; x < buf.byteLength; x++) bstr += String.fromCharCode(buf[x]);
...
function mty_b64_to_buf(b64) {
  ...
  for (let x = 0; x < bstr.length; x++) buf[x] = bstr.charCodeAt(x);
...
function mty_wait(sync) {
  if (Atomics.compareExchange(sync, 0, 0, 1) == 0) Atomics.wait(sync, 0, 1);
...
function mty_add_input_events(thread) {
  ...
  window.addEventListener('keydown', (ev) => {
    ...
    if (MTY.kb_grab || !mty_allow_default(ev)) ev.preventDefault();
  });
...
function mty_set_cursor(url, hot_x, hot_y) {
  if (url) {
    ...
    if (!MTY.defaultCursor) MTY.canvas.classList.add(MTY.cursorClass);
  } else {
    ...
    if (gp) {
      ...
      if (gp.buttons) {
        ...
        for (let i = 0; i < gp.buttons.length && i < 32; i++)
          if (gp.buttons[i].pressed) buttons |= 1 << i;
      }
...
async function mty_audio_queue(
  ctx,
  sampleRate,
  minBuffer,
  maxBuffer,
  ...
  if (!MTY.audioCtx) {
    ...
    await MTY.audioCtx.audioWorklet.addModule(
      baseFile.replace('.js', '-worker.js'),
    );
...
async function mty_ws_connect(url) {
  return new Promise((resolve, reject) => {
    ...
    ws.onopen = () => {
      ...
      tm = setInterval(() => {
        ws.send('__ping__');
      }, 60000); // we've seen timeouts at the 10-minute mark,
...
async function mty_ws_read(ws, timeout) {
  ...
  if (!msg) {
    ...
    if (r1 != 'timed-out') msg = ws.msgs.shift();
  }
...
function mty_update_interval(thread) {
  // Poll gamepads
  if (document.hasFocus()) mty_poll_gamepads(thread);
...
function mty_thread_start(
  ...
  const baseFile = MTY_CURRENT_SCRIPT.pathname;
  const worker = new Worker(baseFile.replace('.js', '-worker.js'), {
    name: name,
  });
...
async function MTY_Start(bin, container, userEnv) {
  if (!mty_supports_web_gl()) return false;
  ...
  // Main thread
  MTY.mainThread = mty_thread_start(
    MTY.threadId,
    bin,
    MTY.wasmBuf,
    MTY_MEMORY,
...
async function mty_thread_message(ev) {
  ...
  switch (msg.type) {
    ...
    case 'uri':
      ...
      break;
    case 'http': {
      const res = await mty_http_request(
        msg.url,
        msg.method,
        msg.headers,
        msg.body,
    ...
    case 'audio-queue':
      ...
      break;
    case 'audio-destroy':
      if (MTY.audioCtx) MTY.audioCtx.close();
      delete MTY.audioCtx;
      break;
    case 'async-copy':
    ...
    case 'wv-navigate':
      if (msg.url) {
        ...
        MTY.webview.onload = () => {
          try {
            const script =
              MTY.webview.contentWindow.document.createElement('script');
            script.textContent =
              "window.parent.postMessage('R');window.MTY_NativeSendText = (text) => { window.parent.postMessage('T' + text); }";
            MTY.webview.contentWindow.document.head.appendChild(script);
            const userAgentScript =
              MTY.webview.contentWindow.document.createElement('script');
            userAgentScript.textContent =
              "Object.defineProperty(window, 'MTY_GetPlatform', { value: () => 'web'});";
            MTY.webview.contentWindow.document.head.appendChild(
              userAgentScript,
            );
            setTimeout(() => (MTY.webview.style.visibility = 'visible'), 250);
          } catch (e) {
            console.error(
              'Failed to inject script into iframe (cross-origin restriction):',
              e,
            );
            setTimeout(() => (MTY.webview.style.visibility = 'visible'), 250);
          }
