...
MTY_Cert *MTY_CertCreate(void)
{
	...
	cert->key = RSA_new();
	RSA_generate_key_ex(cert->key, DTLS_RSA_BITS, bne, NULL);
	BN_free(bne);
	...
	// set1 takes a reference rather than ownership, so the RSA key remains owned by MTY_Cert
	EVP_PKEY *pkey = EVP_PKEY_new();
	EVP_PKEY_set1_RSA(pkey, cert->key);
...
MTY_DTLS *MTY_DTLSCreate(MTY_Cert *cert, const char *peerFingerprint, uint32_t mtu)
{
	...
	// Pin to DTLS 1.2 to match the other platforms and MTY_IsDTLSHandshake's version check
	SSL_CTX_set_max_proto_version(ctx->ctx, DTLS1_2_VERSION);
	...
	// This will tell libssl to create larger datagrams
